...
## Phase 2 storage and sync compatibility (September 2026)

- Backup v4 includes cached events, identity history, change history, metadata, PGCE records and attachment bytes. Validated v2/v3 imports remain supported. Keep a downloaded backup before intentional restore; the import preview identifies replaced profiles and record/file counts. The 50 MB backup cap and 10 MB individual wallet cap are checked before mutation.
- IndexedDB attachment database names, versions and numeric IDs are unchanged. A new optional `uid` makes import idempotent. Legacy attachments receive a content-derived identity when exported/imported; ownership strings remain valid.
- `timetable-recovery/journal` holds an undo snapshot before restore, profile removal or remote application. Failed writes roll back. An interrupted operation is rolled back before React mounts. Never clear browser data to resolve a recovery error; close other tabs, free space, and reload to retry. A separate downloaded backup remains the recovery option if device/browser storage is lost.
- Event owners and calendar UIDs already observed are preserved. New explicit Event ID/Session ID columns provide source identity; otherwise the reconciliation history matches exact rows and uniquely supported moves. Ambiguous candidates remain separate and can be reviewed in session detail. Previously orphaned notes appear in the evidence journal. A source without stable IDs or prior snapshots cannot guarantee identity after arbitrary simultaneous edits; add unique source IDs to resolve that uncertainty across clients.
- Sync v2 adds the `SYNC` SQLite Durable Object binding (`SyncStore`, migration `sync-v2`). Existing KV namespace IDs are unchanged. A code's legacy encrypted KV blob is read once into its object; KV records are not deleted by the migration. Each write compares its revision within one storage transaction, returning 409 on conflict. The client reads, decrypts, merges, encrypts and retries. Photos/wallet bytes still require backup transfer.
- Deploy the worker before the app. `/sync` and `/sync/delete` now return 426 so older clients cannot bypass revision checks. They retain their local data and must refresh the app to resume sync. `/sync-v2` is intentionally unavailable on a Phase 1 worker; the new app reports a failure rather than reverting to unsafe writes.
- Rollback: prefer a compatible forward fix. Preserve eventKey/calendarUid, deletion tombstones, attachment UIDs and the new sync binding. Do not deploy the old last-write-wins worker or restore its `/sync` writes: that would allow deleted records to return. If reverting the UI, retain the Phase 2 storage/sync modules. A v4 export can restore into a compatible app; older app versions do not understand v4 recovery or tombstones. Never restore the old broadcasting `/test` endpoint.
- Local release tests use synthetic records, isolated browser contexts and an in-memory sync service. Production verification remains read-only; it does not send notifications or inspect real sync contents.
