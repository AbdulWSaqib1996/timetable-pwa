...
export interface WorkloadProposal { taskId: string; parentKind?: 'task' | 'homework'; title: string; dateISO: string; startTime: string; endTime: string; effortMins: number }
export function childOf(item: { id: string; planKind?: 'task' | 'homework' }, p: { parentId: string; parentKind?: 'task' | 'homework' }): boolean
