...
> **Status (11 September 2026): implemented as Pass 70** — see PLAN.md. Refinements recorded there: the workbench, practice and preparation sheets are in the main bundle so the cycle works offline after load; rehearsal blocks are a task + timed block in the existing plan model; two built-in lesson templates; Today next steps are per-device dismiss/pin.
