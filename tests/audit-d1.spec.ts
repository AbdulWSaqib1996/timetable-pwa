...
test('E04: preparing for SE2 never writes to SE1, a moved pin makes the travel review stale, and a per-placement return destination drives Back home', async ({ page }) => {
  ...
  await expect(progress).toContainText('4 of 10 done')
  ...
  await expect(progress).toContainText('9 of 10 done')
  // Pass 89: the resources item can be "not known yet" with a follow-up date — it never blocks readiness.
  const resItem = page.getByRole('list', { name: 'Teaching context' }).locator('li').filter({ hasText: 'School resources to hand' })
  await resItem.getByLabel(/Follow up on: School resources/).fill('2027-01-04')
  await resItem.getByRole('button', { name: 'Not known yet' }).click()
  await expect(progress).toContainText('9 of 10 done · 1 not known yet')
  ...
  await page.reload()
  await expect(progress).toContainText('8 of 10 done · 1 not known yet · 1 needs another look')
