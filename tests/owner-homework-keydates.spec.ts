...
test('homework is set from the session detail and assigned to a later occurrence of that session', async ({ page }) => {
  ...
  // Pass 88 (UX-01): homework is its own section of the session detail, and the editor opens on request.
  await page.getByRole('tab', { name: /^Homework/ }).click()
  await page.getByRole('group', { name: 'Homework set in this session' }).getByRole('button', { name: 'Add homework' }).click()
