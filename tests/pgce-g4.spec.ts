...
test('learner and mentor, end to end: invite → join → share exactly the preview → signed feedback becomes read-only reviewer-authenticated → access ended', async ({ page, context }) => {
  ...
  await expect(sheet.getByLabel('Check and share')).toContainText('First share of this pack.')
  await sheet.getByRole('button', { name: 'Share this version' }).click()
  await expect(sheet.getByRole('status')).toContainText('Shared version 1 with 1 mentor')
